...
(function () {
  ...
  async function main(){
    ...
    try {
      ...
      // đúng chủ đề đã lưu (so-tu-nhien) — kiểm nhãn chủ đề hiển thị
...
function runChromeDump() {
  ...
  const args = [
    ...
    '--no-first-run',
    '--disable-extensions',
    // 30s thời gian ẢO (không phải 30s thật): driver bấm qua ~30 câu ở chế độ trộn nên
    // chuỗi setTimeout khá dài. Để 15s thì lúc máy bận (chạy nhiều bộ test liên tiếp)
    // driver bị cắt ngay ở ca kiểm cuối và test FAIL oan 2 assert cuối cùng.
...
export function run() {
  ...
  const expected = [
    'home_visible', 'engine_loaded', 'hook_installed', 'topic_grid_10',
