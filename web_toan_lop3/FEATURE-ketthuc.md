...
### 1.4b TỰ CHẤM "IM LẶNG" câu đang làm khi chuyển tiếp / kết thúc (CẬP NHẬT)

> Bổ sung sau khi phát hiện: bé chọn/nhập đáp án rồi bấm **"Câu tiếp →"** hoặc **"🏁 Kết thúc"** mà QUÊN bấm **"✅ Kiểm tra"** → câu đó trước đây KHÔNG được chấm, KHÔNG vào `records` → màn tổng hợp bỏ sót. Nay sửa: **tự chấm câu đang làm nếu bé ĐÃ CÓ đáp án.**

**Điều kiện "đã có đáp án"** (hàm `hasAnswer()`):
- `mc`: `selectedIndex >= 0` (đã chọn 1 phương án).
- `input`: `answerBox.value.trim() !== ''` (ô nhập khác rỗng).

**Đường chấm "im lặng"** — refactor `submitAnswer()` thành `submitAnswer({silent})` (mặc định `silent=false`); thêm `onCorrect(silent)` / `onWrong(silent)` và helper `gradeIfPending()`:
- Khi `silent=true`: **dùng lại y hệt** logic chấm — `QuestionEngine.check` (mc truyền `selectedIndex`; input truyền `answerBox.value`), `answered=true`, ghi `session.records` (cùng shape, kể cả `say` ở L1), cộng sao đúng/sai, chuỗi 🔥, `bumpDailyCount()`, và ở Ôn tập cập nhật `session.correctCount` / `wrongByTopic` (để X/10 đúng), `checkBadges()`.
- KHÁC chấm thường: **KHÔNG** hiện vùng `#feedback`, **KHÔNG** đổi/ẩn nút, **KHÔNG** confetti/âm thanh, (L1) **KHÔNG** TTS, **KHÔNG** focus `nextBtn`. `onCorrect(true)` chỉ cập nhật điểm rồi `return`; `onWrong(true)` chỉ reset chuỗi rồi `return`.
- `gradeIfPending()` = `if (!answered && hasAnswer()) submitAnswer({silent:true});` — guard `!answered` chống chấm lại / ghi trùng record.

**Nối nút:**
- **"Câu tiếp →" (`skipBtn`)**: `gradeIfPending(); nextQuestion();`. Nếu đã có đáp án → chấm im lặng (ghi record) rồi sang câu mới; nếu chưa có đáp án → bỏ qua thật (KHÔNG ghi record).
- **"🏁 Kết thúc" (`finishBtn` → `finishPractice()`)**: gọi `gradeIfPending()` **trước** khi kiểm tra `records.length === 0` và dựng `renderSummary()` → câu đang làm (nếu đã có đáp án) được tính vào tổng hợp.

Nhờ "Câu tiếp →" giờ ghi record khi đã có đáp án, **mọi câu bé trả lời rồi chuyển tiếp đều nằm trong `session.records`** → màn tổng hợp duyệt đủ cả các câu chưa-bấm-Kiểm-tra (đúng yêu cầu).

**Không phá luồng cũ:** đường "✅ Kiểm tra" → "Câu tiếp theo →" vẫn `answered=true` nên `gradeIfPending()` không làm gì (không ghi trùng). Câu **không có đáp án** khi chuyển tiếp/kết thúc = **bỏ qua thật**, không tính (không chấm sai). Các call site `submitAnswer()` cũ (checkBtn, phím Enter) gọi không tham số → `silent` falsy → chấm thường như trước.
...
### 3.2 KHI NÀO ghi
Ghi **1 lần mỗi câu, tại `submitAnswer()`** ngay sau khi xác định `ok` (đã có biến `ok`, `selectedIndex`, `currentQ`). Đặt **sau** dòng `answered = true;`. **Cả 2 đường chấm đều ghi**: chấm thường (bấm "✅ Kiểm tra") và chấm "im lặng" (`submitAnswer({silent:true})` khi bé bấm "Câu tiếp →"/"🏁 Kết thúc" mà đã có đáp án — xem §1.4b). Guard `!answered` chống ghi trùng.
...
4. **Thêm `session.records = []`**, ghi **1 bản ghi mỗi câu trong `submitAnswer()`** sau khi có `ok`; **reset** ở đầu `startPractice()` và `startMixed()`. Câu "bỏ qua" KHÔNG có đáp án thì KHÔNG ghi; câu ĐÃ CÓ đáp án mà bé chuyển tiếp/kết thúc trước khi Kiểm tra → **tự chấm "im lặng"** và GHI (xem §1.4b). Chỉ ở RAM, KHÔNG localStorage.
...
13. **TỰ CHẤM "IM LẶNG" câu đang làm (CẬP NHẬT — §1.4b)**: refactor `submitAnswer({silent})` + `onCorrect(silent)`/`onWrong(silent)` + helper `hasAnswer()`/`gradeIfPending()`. Khi bé bấm **"Câu tiếp →"** hoặc **"🏁 Kết thúc"** mà đã có đáp án nhưng chưa Kiểm tra → tự chấm (ghi `records`, cộng điểm/sao/chuỗi/huy hiệu, cập nhật mixed) **không feedback/confetti/âm thanh/TTS/focus**. Câu không có đáp án = bỏ qua thật. Guard `!answered` chống ghi trùng; không phá luồng "Kiểm tra → Câu tiếp theo".
