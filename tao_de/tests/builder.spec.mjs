...
function measure(key, muc, runs) {
  ...
  for (let i = 0; i < runs; i++) {
    ...
    de.items.forEach((it) => {
      ...
      if (['tu-duy','loi-van','bieu-thuc','tinh-day','chia-du',
           'toan-dien-hinh','chuyen-dong','ti-so-phan-tram'].indexOf(it.topic) >= 0) tuDuy++;
      if (it.tier === 2) tier2++;
...
/* Số đề dùng để ĐO độ khó. Trước đây là 40 và tỉ lệ đo được của lớp 3 dao động
   quanh ngưỡng 1,9 (đã quan sát 1,86 · 1,93 · 1,96 · 2,01) nên test thỉnh thoảng
   FAIL oan dù engine không đổi. Tăng cỡ mẫu để con số ổn định — đây là phép đo
   thống kê, cỡ mẫu nhỏ thì sai số lớn hơn khoảng cách cần phân biệt. */
const SO_DE_DO = 250;
